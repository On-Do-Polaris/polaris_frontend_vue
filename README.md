...
│  │          │          │                 │ │
│  ├──────────┼──────────┼─────────────────┤ │
│  │ Report   │ Site Mgmt│ My Account      │ │
│  └──────────┴──────────┴─────────────────┘ │
├─────────────────────────────────────────────┤
│            Shared Components                │
│  ┌──────────┬──────────┬─────────────────┐ │
│  │ Charts   │ Analysis │ Simulation      │ │
│  │          │  Tabs    │  Components     │ │
│  ├──────────┼──────────┼─────────────────┤ │
│  │ UI       │ Common   │                 │ │
│  │ (shadcn) │ (Header) │                 │ │
...
이 애플리케이션은 IPCC의 SSP (Shared Socioeconomic Pathways) 시나리오를 기반으로 기후 리스크를 분석합니다.

### SSP 시나리오

| 시나리오     | 설명                                | 온도 상승 | 리스크 수준 |
| ------------ | ----------------------------------- | --------- | ----------- |
| **SSP1-2.6** | 지속가능한 발전, 파리협정 목표 달성 | +1.5°C    | 낮음        |
| **SSP2-4.5** | 중간 경로, 현재 정책 유지           | +2.7°C    | 중간        |
| **SSP3-7.0** | 지역 분열, 기후 정책 실패           | +3.6°C    | 높음        |
| **SSP5-8.5** | 화석연료 중심 발전                  | +4.4°C    | 매우 높음   |
